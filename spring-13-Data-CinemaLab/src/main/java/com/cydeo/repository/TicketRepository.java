...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    //Write a derived query to count how many tickets a user bought
     Integer countAllByUserAccountId(Long user_id);

    //Write a derived query to list all tickets by specific email
      List<Ticket>getAllByUserAccountEmail(String email);
    ...
    List<Ticket>getAllByDateTimeBetween(LocalDateTime dateTime, LocalDateTime dateTime2);

    // ------------------- JPQL QUERIES ------------------- //

    //Write a JPQL query that returns all tickets are bought from a specific user
    @Query("select t from Ticket t where t.userAccount.id = ?1")
     List<Ticket>getByUserAccount(@Param("userId")Long userId);

    //Write a JPQL query that returns all tickets between a range of dates
    @Query("select t from Ticket t where t.dateTime between ?1 and ?2")
    List<Ticket>getAllByRangeOfDate(@Param("dateTime1")LocalDateTime dateTime1,@Param("dateTime2")LocalDateTime dateTime2);

    // ------------------- Native QUERIES ------------------- //

    //Write a native query to count the number of tickets a user bought
     @Query(value = "select count(*) from Ticket t where t.user_account_id = ?1",nativeQuery = true)
     Integer getAllTicketsByUserAccount(@Param("userId")Long userId);
